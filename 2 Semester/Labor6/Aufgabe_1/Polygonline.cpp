...
//--------------------------------------------------------------------------------------
void Polygonline::print(bool value) const
{
	map<PlgElement*, Point> plgElementMap;
	...
	while (iterator)
	{
		...
		if (plgElementMap.count(iterator) == 0)
		{
			plgElementMap.insert(pair<PlgElement*, Point>(iterator, iterator->getPoint()));
		}
		else
		{
			throw LoopInLine(m_ID);
		}
